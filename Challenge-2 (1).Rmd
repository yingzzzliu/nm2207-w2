...
**Solution:** _Comma separated values, it is broadly compatible and is able to be easily read and processed._ 
<br>
<br>
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="SS of files pane "}
knitr::include_graphics("C:/Users/angel/OneDrive/Pictures/Screenshots/Screenshot (150).png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** _It skips the rows with missing values._ 
<br>
<br>
...
ggplot(playlist_data) + aes(x=Age,y=Rating) + labs(x="Age",y="Rating") + geom_point()
...
ggplot(playlist_data) + aes(x=Age,y=Rating) + labs(x="Age",y="Rating") + geom_point(shape = 17)
...
ggplot(playlist_data) + aes(x=Age,y=Rating) + geom_point(shape = 17) + labs(x="Age",y="Rating",title="Plot of DJs' age and ratings",caption="There is no direct relationship between the age of DJs and their ratings. ")
